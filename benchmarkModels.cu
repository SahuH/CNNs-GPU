...
int main(int argc, char** argv) {
    ...
    for(int i = 0; i < n_conv_layers; i++) {
        ...
        add_padding(input, output, padding);
        cudaFree(input);
        input = output;

        // printf("Padding done\n");

        convolution(input, output, kernels[i], CONV_STRIDE);
        cudaFree(input);
        input = output;
    ...
    for(int i = 0; i < n_pooling_layers; i++) {
        pooling(input, output, POOLING_SIZE, POOLING_STRIDE);
        cudaFree(input);
        input = output;
    ...
    for(int i = 0; i < n_dense_layers; i++) {
        dense(input, dense_layer_output, dense_layers_weights[i], dense_layers_biases[i], n_neurons);
        cudaFree(input);
